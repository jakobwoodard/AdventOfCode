...
def formatDisk(disk_map):
    l_pointer = 0
    r_pointer = len(disk_map) - 1
    while l_pointer < r_pointer:
        if not disk_map[l_pointer] == '.':
            l_pointer+=1
        if disk_map[r_pointer] == '.':
            r_pointer-=1
        if disk_map[l_pointer] == '.' and not disk_map[r_pointer] == '.':
            disk_map[l_pointer] = disk_map[r_pointer]
            disk_map[r_pointer] = '.'
